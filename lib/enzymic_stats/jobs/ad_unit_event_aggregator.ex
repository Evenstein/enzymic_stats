defmodule EnzymicStats.Jobs.AdUnitEventAggregator do
  require Logger
  import EnzymicStats.Helpers.AggregatorHelper
  import Ecto.Query, only: [from: 2, where: 3]

  alias EnzymicStats.Models.{AdUnit, Campaign, AdUnitReport, AdUnitCurrentReport}
  ...
  defp report_for_ad_unit_sizes(ad_unit, data, date, _default) do
    [content_type, size, ad_unit_id] = ad_unit

    date = transform_date_ad_unit(Repo.get(AdUnit, ad_unit_id), date)

    iteration = fn([category_name, _category]) ->
    ...
    event_types = [["impressions",  0], ["clicks", 1], ["swipes", 2], ["5s_views", 3],
                    ["50%_views", 4], ["100%_views", 5], ["submits", 6], ["likes", 7],
                    ["tw_shares", 8], ["fb_shares", 9], ["in_shares", 10], ["wa_shares", 11],
                    ["email_shares", 12], ["bottom_swipes", 13]]
    ...
    {method, changeset} = AdUnitReport.get_or_build_by(
      %{category: AdUnitReport.category(:ad_unit_size), category_id: ad_unit_id, content_type: content_type,
        size: size, date: Timex.parse(date, "{D}-{0M}-{YYYY}"), data: data})
  ...
  defp report_for_ad_units(ad_unit_id, data, date, default) do
    ...
    {method, changeset} = AdUnitReport.get_or_build_by(
      %{category: AdUnitReport.category(:ad_unit), category_id: ad_unit_id,
        date: Timex.parse(date, "{D}-{0M}-{YYYY}"), data: results})
    apply Repo, method, [changeset]
  end
