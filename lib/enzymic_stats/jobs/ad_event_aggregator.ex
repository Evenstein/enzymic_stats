defmodule EnzymicStats.Jobs.AdEventAggregator do
  require Logger
  import EnzymicStats.Helpers.AggregatorHelper
  import Ecto.Query, only: [from: 2, where: 3]

  alias EnzymicStats.Models.{AdUnit, Campaign, Report, CurrentReport}
  ...
  defp campaign_report(campaign, data, date \\ Timex.now, default) do
#    date = transform_date_campaign(campaign, date)
    ...
    {method, changeset} = Report.get_or_build_by(%{"category" => "campaign", "category_id" => campaign.original_id, "date" => date, "data" => results})
    apply Repo, method, [changeset]
  end

  defp ad_unit_report(ad_unit, data, date \\ Timex.now, default) do
#    date = transform_date_ad_unit(ad_unit, date)

    results = calculate_ad_events(ad_unit.ads, data, default)
    {method, changeset} = Report.get_or_build_by(%{"category" => "ad_unit", "category_id" => ad_unit.original_id, "date" => date, "data" => results})
    apply Repo, method, [changeset]
  end

  defp ad_report(ad_id, data, date \\ Timex.now) do
    ad_unit = Repo.one(from au in AdUnit, where: fragment("? && ?", au.ads, ^[ad_id]))
#    date = transform_date_ad_unit(ad_unit, date)

    results = Map.get(data, ad_id)
    {method, changeset} = Report.get_or_build_by(%{"category" => "ad", "category_id" => ad_id, "date" => date, "data" => results})
    apply Repo, method, [changeset]
  end
