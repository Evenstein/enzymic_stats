defmodule EnzymicStats.Models.AdUnitReport do
  require Logger
  use Ecto.Schema
  ...
  alias EnzymicStats.Models.{AdUnit, AdUnitReport}
  ...
  schema "ad_unit_reports" do
    ...
    field :size, :integer
    field :date, :utc_datetime #Timex.Ecto.Date
    field :data, :map
  ...
  def get_or_build_by(params) do
    ...
    case AdUnitReport
      |> Repo.get_by(report_params) do
        nil ->
          {:insert, changeset(struct(AdUnitReport, %{}), params)}
        report ->
  ...
  def get_ad_unit_size_stats(ad_unit_id, date_start \\ :empty, date_finish \\ :empty) do
    ad_units = Repo.all(from r in AdUnitReport,
  ...
  defp get_stats(category, category_id, date_start, date_finish, content_type \\ nil, size \\ nil) do
    query = if category == category(:ad_unit_size) do
      AdUnitReport |> where([r], r.category == ^category and
        r.category_id == ^category_id and
        r.content_type == ^content_type and
        r.size == ^size)
    else
      AdUnitReport |> where([r], r.category == ^category and r.category_id == ^category_id)
    end
  ...
  defp default do
    struct(AdUnitReport, %{data: AdUnit.event_types})
  end
end
