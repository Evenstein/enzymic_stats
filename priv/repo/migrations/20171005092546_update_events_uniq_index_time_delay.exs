defmodule EnzymicStats.Repo.Migrations.UpdateEventsUniqIndexTimeDelay do
  use Ecto.Migration
  ...
  def up do
    ...
    SELECT date_trunc('hour', $1) + INTERVAL '2 min' * ROUND(date_part('minute', $1) / 2.0)
    ...
    execute("CREATE UNIQUE INDEX ad_unit_events_unique_index ON ad_unit_events(category, ad_unit_id,
      size, content_type, ad_id, round_time(inserted_at), client_uid) WHERE client_uid IS NOT NULL")
  end
